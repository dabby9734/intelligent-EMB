...
export default function Home() {
  ...
  const fetchMessages = async (type) => {
    ...
    localStorage.setItem(`serviceBoard+${type}`, JSON.stringify(data.messages));
    if (data.name) {
  ...
  useEffect(() => {
    if (type) {
      try {
        setMessages(JSON.parse(localStorage.getItem(`serviceBoard+${type}`)));
      } catch (err) {
        console.log(err);
      }
